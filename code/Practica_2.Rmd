...
```
Veiem que les dades no segueixen una distrubució normal. Per tant, les haurem de tractar.
...
* Hi ha zones amb preus marcadament diferents de la resta?
```{r}
# Imprimir 10 registres més cars i barats de Gasoil
head(subset(data[order(data$Price, decreasing = FALSE),], Fuel_type == "Gasóleo A habitual", select = c("Price", "Brand", "Province", "Fuel_type")), 10)

head(subset(data[order(data$Price, decreasing = TRUE),], Fuel_type == "Gasóleo A habitual", select = c("Price", "Brand", "Province", "Fuel_type")), 10)

# Imprimir 10 registres més cars i barats de Gasolina
head(subset(data[order(data$Price, decreasing = FALSE),], Fuel_type == "Gasolina 95 E5", select = c("Price", "Brand", "Province", "Fuel_type")), 10)

head(subset(data[order(data$Price, decreasing = TRUE),], Fuel_type == "Gasolina 95 E5", select = c("Price", "Brand", "Province", "Fuel_type")), 10)
```


* La mitjana dels preus dels combustibles són diferents entre Barcelona i Madrid?
```{r}
mean_price <- aggregate(Price ~ City, data = capital_filter, mean)
print(mean_price)
```
...
* La marca de la gasolinera influeix en els preus del combustible?
...
mean_price <- aggregate(Price ~ Brand_group, data = gasoleo_a_habitual, function(x) round(mean(x, na.rm = TRUE),3))
other_mean <- round(mean(gasoleo_a_habitual[gasoleo_a_habitual$Brand %in% names(brands_lt_10), "Price"], na.rm = TRUE), 3)

mean_price <- rbind(mean_price, c("Other", other_mean))
ggplot(data = mean_price, aes(x = Brand_group, y = Price)) +
  geom_bar(stat = "identity", color = "black", fill = mypalette[3]) +
  ggtitle("Preu mitjà del Gasoil A per marca") + 
  xlab("Marca de gasolinera") + 
  ylab("Preu mitjà")
```
...
mean_price <- aggregate(Price ~ Brand_group, data = gasolina95, function(x) round(mean(x, na.rm = TRUE),3))
other_mean <- round(mean(gasolina95[gasolina95$Brand %in% names(brands_lt_10), "Price"], na.rm = TRUE), 3)

mean_price <- rbind(mean_price, c("Other", other_mean))
ggplot(data = mean_price, aes(x = Brand_group, y = Price)) +
  geom_bar(stat = "identity", color = "black", fill = mypalette[1]) +
