...
```{r, vspaceecho='10pt'}
boxplot(Price ~ Pop_size, data = pop_filter_fuel, main = "BoxPlot Gasolina 95 E5",
        xlab = "Mida del municipi", ylab = "Preu")

```

\newpage
Els resultats d'aquests primers anàlisis mostren que:
...
* La mida del municipi (petit, mitjà o gran) té una influencia estadisticament significativa en el preu dels dos combustibles en estudi.



A continuació es vol estudiar si els preus del combustible estan correlacionats amb el nombre d'estacions de servei per municipi.
El primer pas és la creació d'una sèrie de df i grups de dades que ens ajuden en l'anàlisi.
```{r, vspaceecho='10pt'}
#Data subset
...
Seguidament durem a terme una regressió lineal per analitzar la relació.
...
```{r}
gasolinab$Brand_num <- as.numeric(gasolinab$Brand)
cor.test(gasolinab$Price, gasolinab$Brand_num, method = "spearman")
```
...
Com que en ambdos casos obtenim p-valors menors que el nivell de significació (0.05), es pot concloure que hi ha, com a mínim, un grup (marca d'estació de servei) estadísticament diferent dels altres grups en per cada combustible.



A continuació es presenta un histograma amb l'objectiu de coneixer la distribució de preus d'aquest darrer anàlisi.

\vspace{10pt}

```{r echo=FALSE, fig.height=3.75, message=FALSE, warning=FALSE}
...
* [vídeo](https://drive.google.com/file/d/12ABq1iRGKkafQVI00XEbas6sz7IEszlN)
