...
for (brand in Brand.names){
  total$Brand.factor<-if_else(grepl(brand, total$Brand.factor), brand, total$Brand.factor)
}
#Assignació de camp "OTROS"
total$Brand.factor<-if_else(total$Brand.factor %in% Brand.names, total$Brand.factor, "OTROS")
...
NOTA: L'estudi d'*outliers* en l'extrem superior es podria allargar força més considerant la consistència de registres en l'horitzó tempora, considerant la consistència de registres d'una mateixa marca en una zona pròxima ...
...
Per finalitzar l'anàlisi dels valors extrems inferiors, es procedeix d'acord al mètode de selecció utilitzat previament en els valors extrems superiors. Així doncs, s'obtenen els registres que són valors extrems inferiors tant pel cas del combustible *Gasóleo A habitual* com pel *Gasolina 95 E5*.
...
```{r}
LOWs<-merge(DiesLow, GasLow, by=c("Province", "City", "Capture_date", "Address", "Brand.factor"),
           all = TRUE)
LOWs<-LOWs[c("Province", "City", "Address", "Brand.factor", "Fuel_type.x", "Price.x",
           "Fuel_type.y", "Price.y")] %>%
  arrange(Province, City, Brand.factor, Address)
```
...
```{r}
pbrand <- lm(Price ~ Brand, data = total)
summary(pbrand)
```
